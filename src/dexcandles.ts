...
function getTokenAmount(token: string, amountIn: BigInt, amountOut: BigInt): BigInt {
  ...
  if (exponent >= BigInt.fromI32(0)) {
    ...
    const divider = BigInt.fromString(
      BigDecimal.fromString('1e' + exponent.times(BigInt.fromI32(-1)).toString()).toString()
    )
...
export function handleSwap(event: Swap): void {
  ...
  const token0Amount: BigInt = getTokenAmount(
    pair.token0.toHexString(),
    event.params.amount0In,
    event.params.amount0Out
  )
  const token1Amount: BigInt = getTokenAmount(
